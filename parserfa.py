...
class Terminal:
    def __init__(self, type, verbatim=True):
        ...
        # symbol and keyword tokens need to be matched verbatim, others just need to match token type
...
class NonTerminal:
    # TODO: probably func something general (apply fa) since fa is passed as well
    def __init__(self, predict=[], follow=[], func=None, name="", fa=None):
        # a list of type "Terminal" (first of nonterminal + follow of nonterminal if epsilon in first)
  ...
  #      return None
    ...
    def nextState(self, from_state, token):  # returns (next state, produced parse tree)
        ...
        for f in link[1].follow:
            if f.matches(token):
                # token in follow (self.nt)
                return (SyntaxError.MISSINGNT, link)
...
class Tree:
    ...
    def str_aux(self, tab, last, first):
        ...
        while i < len(self.children):
            ...
            res += child.str_aux((tab if first else (tab + ("    " if last else "│   "))),
