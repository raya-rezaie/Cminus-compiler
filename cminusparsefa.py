...
def cminusParseFA(apply_fa, semantic_action):
    ...
    # 2. DECLARATION LIST
    create_fa([[declaration, declaration_list], [None]], declaration_list, semantic_action)

    # 3. DECLARATION
    create_fa([[declaration_initial, declaration_prime]], declaration, semantic_action)

    # 4. DECLARATION INITIAL
    create_fa([[actionNames.push_ss,type_specifier, ID]], declaration_initial, semantic_action)
    ...
    # 9. PARAMS
    create_fa([[actionNames.push_ss, INT,actionNames.push_ss, ID, param_prime, param_list], [VOID]], params, semantic_action)

    # 10. PARAM LIST
    create_fa([[COMMA, param, param_list], [None]], param_list, semantic_action)

    # 11. PARAM
    create_fa([[declaration_initial, param_prime]], param, semantic_action)

    # 12. PARAM PRIME
    create_fa([[OPENBRACKET, CLOSEBRACKET , actionNames.save_param_list], [None , actionNames.save_param_norm]], param_prime, semantic_action)

    # 13. COMPOUND STMT
    create_fa([[OPENCURLY,actionNames.save_scope,  declaration_list, statement_list, actionNames.fill_break ,CLOSECURLY]], compound_stmt, semantic_action)

    # 14. STATEMENT LIST
    create_fa([[statement, statement_list], [None]], statement_list, semantic_action)

    # 15. STATEMENT
    create_fa([[expression_stmt], [compound_stmt], [selection_stmt], [iteration_stmt], [return_stmt]], statement, semantic_action)

    # 16. EXPRESSION STMT
    create_fa([[expression, SEMICOLON], [BREAK, SEMICOLON , actionNames.save_jmp_out_scope], [SEMICOLON]], expression_stmt, semantic_action)

    # 17. SELECTION STMT
    create_fa([[IF, OPENPAR, expression,CLOSEPAR,actionNames.save_while_cond_jpf ,statement, ELSE, statement]], selection_stmt, semantic_action)

    # 18. ITERATION STMT
    create_fa([[WHILE, OPENPAR, expression, CLOSEPAR, statement]], iteration_stmt, semantic_action)

    # 19. RETURN STMT
    create_fa([[RETURN, return_stmt_prime]], return_stmt, semantic_action)

    # 20. RETURN STMT PRIME
    create_fa([[actionNames.return_jp ,SEMICOLON], [expression,actionNames.save_return_value ,SEMICOLON]], return_stmt_prime, semantic_action)

    # 21. EXPRESSION
    create_fa([[simple_expression_zegond], [actionNames.pid, ID, b, actionNames.print]], expression, semantic_action)

    # 22. B
    create_fa([[EQUAL, expression, actionNames.assign], [OPENBRACKET, expression, CLOSEBRACKET, actionNames.calc_arr_addr, h], [simple_expression_prime]], b, semantic_action)

    # 23. H
    create_fa([[EQUAL, expression, actionNames.assign], [g, d, c]], h, semantic_action)

    # 24. SIMPLE EXPRESSION ZEGOND
    create_fa([[additive_expression_zegond, c]], simple_expression_zegond, semantic_action)

    # 25. SIMPLE EXPRESSION PRIME
    create_fa([[additive_expression_prime, c]], simple_expression_prime, semantic_action)

    # 26. C
    create_fa([[relop, additive_expression, actionNames.relation], [None]], c, semantic_action)

    # 27. RELOP
    create_fa([[actionNames.push_ss, LESS], [actionNames.push_ss, DOUBLEEQUAL]], relop, semantic_action)
    ...
    # 31. D
    create_fa([[addop, term, actionNames.add_or_sub, d], [None]], d, semantic_action)

    # 32. ADDOP
    create_fa([[actionNames.push_ss, PLUS], [actionNames.push_ss, MINUS]], addop, semantic_action)
    ...
    # 36. G
    create_fa([[MULT, signed_factor, actionNames.mult, g], [None]], g, semantic_action)

    # 37. SIGNED FACTOR
    create_fa([[PLUS, factor], [MINUS, factor], [factor]], signed_factor, semantic_action)

    # 38. SIGNED FACTOR PRIME
    create_fa([[factor_prime]], signed_factor_prime, semantic_action)

    # 39. SIGNED FACTOR ZEGOND
    create_fa([[PLUS, factor], [MINUS, factor], [factor_zegond]], signed_factor_zegond, semantic_action)

    # 40. FACTOR
    create_fa([[OPENPAR, expression, CLOSEPAR], [actionNames.pid, ID, var_call_prime], [actionNames.push_num_ss, NUM]], factor, semantic_action)

    # 41. VAR CALL PRIME
    create_fa([[actionNames.start_args, OPENPAR, args, CLOSEPAR, actionNames.check_args], [var_prime]], var_call_prime, semantic_action)

    # 42. VAR PRIME
    create_fa([[OPENBRACKET, expression, CLOSEBRACKET, actionNames.calc_arr_addr], [None]], var_prime, semantic_action)

    # 43. FACTOR PRIME
    create_fa([[actionNames.start_args, OPENPAR, args, CLOSEPAR, actionNames.check_args], [None]], factor_prime, semantic_action)

    # 44. FACTOR ZEGOND
    create_fa([[OPENPAR, expression, CLOSEPAR], [actionNames.push_num_ss, NUM]], factor_zegond, semantic_action)
    ...
    # 47. ARG LIST PRIME
    create_fa([[COMMA, expression, arg_list_prime], [None]], arg_list_prime, semantic_action)
